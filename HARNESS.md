...
## Security & Professionalism Primitives

**Status**: Curated for the main Agent Harness project  
**Source**: Deep research on affaan-m/ECC (182k+ stars) + shareAI-lab/learn-claude-code (62k+ stars) — May 2026  
**Rule**: These are architectural principles only. Never copy implementations. Adopt the patterns that reduce attack surface.

### The 10 Curated Patterns

**1. 3-Gate Permission Pipeline (Mandatory)**  
Every tool action passes three gates in fixed order: (1) Hard Deny List, (2) Contextual Rules, (3) Human Approval when triggered.  
*Security benefit*: Creates fail-closed by default. Directly mitigates prompt injection → shell execution and destructive operations.

**2. Least Agency + Explicit Trust Boundaries**  
The agent always operates with the smallest possible set of powers and identities. It never inherits the operator’s full credentials or context.

**3. Sanitization as First-Class Layer**  
All inbound content (Unicode, attachments, links, skills, rules, MCPs) passes through structured sanitization before entering context. Treat data as executable context.

**4. Hook System with Runtime Controls**  
Hooks must support execution profiles, selective runtime disablement, and external script implementation (never inline).

**5. Strategic Context Compaction**  
Context reduction is a deliberate first-class mechanism with retention policies and explicit security goals — not an emergency hack.

**6. Memory Threat Model ("Memory as Gasoline")**  
Persistent memory is recognized as an attack vector. Requires explicit policies for rotation, trust scoring, isolation, and controlled reset.

**7. Skills / Rules / Hooks Treated as Supply Chain**  
All extensions are supply-chain artifacts. They require review, hashing, provenance, and scanning before loading.

**8. Security Scanning as Mandatory Gate**  
Automated scanning (hooks, MCPs, permissions, secrets, prompt injection patterns) must exist as a gate before execution.

**9. Continuous Learning with Confidence Scoring**  
Evolution of rules, instincts, or patterns must carry confidence scores and human review before production use.

**10. Cross-Harness Design by Default**  
The harness must be architected from the start to be portable and comparatively auditable across different platforms and vendors.

### Integration Guidance
- These patterns were selected for maximum security ROI with minimal bloat.
- Full research report, rationale, and gap analysis: `docs/research/secure-agent-harness-patterns/RESEARCH-REPORT.md`
- Quick reference: `docs/research/secure-agent-harness-patterns/QUICK-REFERENCE.md`
- Reload full context in any future session with the skill: `secure-agent-harness-patterns`

### Meta-Governance
The standard itself is governed through a formal meta-framework:
- `docs/harness-governance/HARNESS-META-GOVERNANCE.md` — How the HARNESS evolves, makes decisions, and protects itself.
- `docs/harness-governance/HARNESS-THREAT-MODEL.md` — Threat model of the standard and its artifacts.
- `docs/harness-governance/HARNESS-DECISIONS.md` — Log of major architectural and security decisions with rationale.

This meta-layer is required for the project to become a living, trustworthy global reference rather than a static document.

### Security Enforcement (Operationalization)
The 10 primitives are not optional suggestions. They must be **enforced** through concrete gates.

- Every harness (and major updates to this standard) **must** pass the [Security Gate Checklist](docs/harness-security/SECURITY-GATE-CHECKLIST.md) before being considered production-grade.
- The checklist is deliberately lightweight but non-negotiable on the Critical items.
- Future versions of this standard will define progressive Security Levels (Level 0–3) based on how completely the primitives are implemented.

### Defensive Posture
In addition to controlling external software safely, agents built with this standard must protect **themselves**.

See: `docs/harness-security/DEFENSIVE-AGENT-POSTURE.md` for the current framework and maturity levels.

**Pilot progress (HarnessQwen-v6.5)**: Level 0 baseline advancing.
- Output Sanitizer (prompt injection defense) + enforced in .clinerules
- Context Guard (memory integrity + poisoning detection)
...
A serious harness standard must be measurable and continuously improvable.

See: `docs/harness-governance/HARNESS-EVALUATION.md` for the current framework.

### Adoption & Ecosystem
A world-class standard needs clear maturity levels and a healthy contribution model.

See: `docs/harness-governance/HARNESS-ADOPTION-AND-LEVELS.md`

### Why This Standard
If you're evaluating whether to adopt this approach:

See: `docs/harness-governance/WHY-THIS-STANDARD.md` (one-page positioning for executives and new users).

### 2026 Roadmap
This standard has a clear, ambitious plan to become a world-class reference.

See: `docs/harness-governance/HARNESS-ROADMAP-2026.md`

**30-Day Sprint Progress** (as of 2026-05-25):
- Security Gate Checklist applied as pilot on HarnessQwen-v6.5 → see `docs/harness-security/SECURITY-GATE-PILOT-HarnessQwen-v6.5.md`
- First version of Harness Evaluation Report template created
