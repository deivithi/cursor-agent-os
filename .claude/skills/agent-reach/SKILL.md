...
description: >
  ...
  PT-BR (auto): material solto, lê este link, abre este link, lê este tweet,
  x.com, twitter.com, Google Docs, YouTube, Reddit, Instagram, Facebook,
  GitHub, Bilibili, Xiaohongshu, RSS, página web — fetch via Agent Reach
  (não inventar WebFetch). Usado pelo Passo 0 de material-solto.

  15 platforms, multi-backend routing (OpenCLI / per-platform CLIs / APIs).
  Zero config for 6 channels. Run `agent-reach doctor --json` to see which
  backend serves each platform right now. Local target: 12/15 (not Groq/Xueqiu/LinkedIn).
...
triggers:
  - research: 调研/全网调研/帮我调研/研究一下/research/深入了解/pesquisar/pesquisa
  - search: 搜/查/找/search/搜索/查一下/帮我搜/看看大家怎么说/buscar/procurar
  - material-solto: material solto/lê este link/abre este link/lê este tweet/link solto/arquivo solto
  - social:
    - 小红书: xiaohongshu/xhs/小红书/红书
    - Twitter: twitter/推特/x.com/twitter.com/推文/tweet
    - B站: bilibili/b站/哔哩哔哩
    - V2EX: v2ex
    - Reddit: reddit
  ...
  - docs: google docs/docs.google.com/documento google
  - dev: github/代码/仓库/gh/issue/pr/分支/commit
  - web: 网页/链接/文章/rss/读一下/打开这个/página/artigo
...
## Local status (Windows — deivithi.lopes)

| Item | Value |
|------|--------|
| **Version** | Agent Reach `v1.5.0` |
| **Target score** | **12/15** channels (Doc end-state; not 15/15) |
| **Venv / CLI** | `C:\Users\deivithi.lopes\.agent-reach-venv\Scripts\` |
| **Config** | `C:\Users\deivithi.lopes\.agent-reach\config.yaml` |
| **mcporter** | home scope: `C:\Users\deivithi.lopes\.mcporter\mcporter.json` |
| **OpenCLI** | `@jackwener/opencli` ≥ `1.8.6` |

### Active (12) — do not invent offline channels

Web (Jina), YouTube (yt-dlp), GitHub (`gh`), RSS, Exa (`mcporter`), V2EX, Bilibili, 小红书, Twitter/X, Reddit, Facebook, Instagram (socials via OpenCLI / cookies as configured).

### Offline by design (3)

小宇宙/Groq, 雪球, LinkedIn — out of Doc scope; do not claim available.

### PowerShell (Windows)

```powershell
$env:Path = "$env:USERPROFILE\.agent-reach-venv\Scripts;" + (npm config get prefix) + ";" + $env:Path
agent-reach doctor
agent-reach doctor --json
opencli doctor
```

Never install/clone Agent Reach inside the `VS CODE` workspace. Temp output → `$env:TEMP`; persistent → `~\.agent-reach\`.
